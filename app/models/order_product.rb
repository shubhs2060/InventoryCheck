...
belongs_to :order
  belongs_to :product
  has_many :order_product_histories, autosave: true, dependent: :destroy
  before_create :build_order_product_histories
  ...
  # parse quantity to build line products of packs
  def build_order_product_histories
    return if product.nil?

    denominations = product.product_packs.pluck(:quantity).sort!.reverse
    ...
    product.product_packs.order("quantity DESC").each do |pack|
      params = {
        product_pack_id: pack.id,
        quantity: quantities[pack.quantity],
        price_per_pack: pack.price
      }
      order_product_histories << OrderProductHistory.new(params) if quantities[pack.quantity] > 0
    end
